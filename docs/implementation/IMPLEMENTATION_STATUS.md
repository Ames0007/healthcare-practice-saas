...
| UI-004B | COMPLETE | (pending) | Patient Rendez-vous tab: upcoming/history appointments derived from Agenda's mock fixtures by patientId, status-group filters, reused AppointmentCard/AppointmentDrawer (extended, not duplicated), + Nouveau RDV/Ouvrir dans l'agenda navigation (mock data only, no Treatments/Finance/Dossier Santé, no backend integration) |
